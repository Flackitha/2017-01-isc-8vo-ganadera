...
$pdf->Cell(0,0,"			FONDO DE ASEGURAMIENTO GANADERO",0,1,'L');
$pdf->SetFont("helvetica", "B", 12);
$pdf->Cell(0,0,"			"."RAZON SOCIAL     FONDO DE ASEGURAMIENTO UGRCAMP",0,1,'L');
$pdf->SetFont("helvetica", "B", 14);
$pdf->Cell(0,0,"			DOMICILIO       Calle 53, Colonia, ESPERANZA, cp. 24350",0,1,'L');
$pdf->Cell(0,0,"			R.F.C.         FAU121127PT3",0,1,'L');

$pdf->SetFont("helvetica", "B", 20);
$pdf->Cell(0,1,"_______________________________________________",0,1,'C');
...
//DESCRIPCION DEL APARTADO 10 ANEXO-1A
...
$pdf->SetFont("zapfdingbats", "", 10);
if ($model->d_identificacion=="S")
	$pdf->Cell(0.5,1,TCPDF_FONTS::unichr(52),1,0,'C', false,"",0,false,"T","M");
else
	$pdf->Cell(0.5,2,TCPDF_FONTS::unichr(0),1,0,'C', false,"",0,false,"T","M");
	$pdf->SetFont("helvetica", "", 10);
...
$pdf->SetFont("zapfdingbats", "", 10);
if ($model->d_constancia_fondo=="S")
	$pdf->Cell(0.5,1,TCPDF_FONTS::unichr(52),1,0,'C', false,"",0,false,"T","M");
else
	$pdf->Cell(0.5,1,TCPDF_FONTS::unichr(0),1,0,'C', false,"",0,false,"T","M");
	$pdf->SetFont("helvetica", "", 10);
	$pdf->MultiCell(0,1,"Original de la constancia expedida por el Fondo de Aseguramiento				en la que se describen los servicios de aseguraiento que no puede otorgarme y las razones de esa imposibilidad, en su caso. ",1,'L',0,1, "","",true);

$pdf->SetFont("zapfdingbats", "", 10);
if ($model->d_acta_asamblea=="S")
	$pdf->Cell(0.5,1,TCPDF_FONTS::unichr(52),1,0,'C', false,"",0,false,"T","M");
else
	$pdf->Cell(0.5,1.5,TCPDF_FONTS::unichr(0),1,0,'C', false,"",0,false,"T","M");
	$pdf->SetFont("helvetica", "", 10);
	$pdf->MultiCell(0,1.5,"Copia del acta de la Asamblea General del Fondo de Aseguramiento 			en la que se acordo mi separación como socio; o, en su defecto, el original del acuse de la solicitud de separación recibida por el Fondo de Aseguramiento				",1,'L',0,1, "","",true);

$pdf->SetFont("zapfdingbats", "", 10);
if ($model->d_estudio_viabilidad=="S")
	$pdf->Cell(0.5,1,TCPDF_FONTS::unichr(52),1,0,'C', false,"",0,false,"T","M");
else
	$pdf->Cell(0.5,1.5,TCPDF_FONTS::unichr(0),1,0,'C', false,"",0,false,"T","M");
	$pdf->SetFont("helvetica", "", 10);
	$pdf->MultiCell(0,1.5,"Estudio de viabilidad técnico-económico por cultivo o especies ganadera, en su caso, en el que se precisan los índices de indemnizaciones de los últimos cinco años, así como la información necesaria para determinar mi adquision como socio. ",1,'L',0,1, "","",true);
...
$pdf->SetFont("helvetica", "", 12);
$pdf->Cell(0,0,$model->nombre ." ".$model->apellidopaterno ." " .$model->apellidomaterno ,0,1,'C');
